...
class SerialDisplay:
    ...
    def push(self, printers):
        ...
        Порожній список НЕ приймаємо: це майже завжди збій зчитування (скрап
        віддав 0 карток, клієнт не на тому роуті), а не ферма без принтерів.
        Інакше матриця показувала б бадьоре "PRINT 0 ERR 0 IDLE 0 DONE 0"
        замість чесного NO LINK. Старий кадр просто застаріє за stale_after.
        """
        if not printers:
            logger.debug("📟 Дисплей: порожній список принтерів, кадр не оновлюю")
            return
